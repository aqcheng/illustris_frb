...
galaxy_mapdir = '/home/tnguser/frb_project/data/galaxy_maps'
outpath = '/home/tnguser/frb_project/data/galaxy_catalogs/test'
...
def get_cone_shell(sim, snap, origin, outpath, checkpoints, in_region, kwargs=None,
    ...
    print(f'{"":<8}{len(boxes)} periodic box(es) found')
    ...
    for box in boxes:
        ...
        print(f'{time.time()-start:<6.0f}: Converting to spherical coordinates')       
        rel_coords = data[:,:3] + box*sim.boxsize - origin
        theta, phi = vec2ang(rel_coords)
        
        print(f'{time.time()-start:<6.0f}: Computing distances')    
        dists = norm(rel_coords, axis=1) 
        
        print(f'{time.time()-start:<6.0f}: Computing mask') 
    ...
    if not os.path.isfile( metadata_path ):
        with open(metadata_path, 'w+') as f:
            ...
            f.write(f"{'snap':<6}{'min_x':<16}{'max_x':<16}{'min_z':<10}{'max_z':<10}\n")
    with open(os.path.join(outpath, metadata), 'a+') as f:
        f.write(f'{snap:<6}{min_x:<16.6f}{max_x:<16.6f}{min_z:<10.6f}{max_z:<10.6f}\n')
    
    df = None #clear

x_lims = np.insert(np.convolve(sim.snap_xs, [0.5,0.5], 'valid'), 99, 0) 
...
mask_kwargs = {'theta_min': theta_min,
               ...
               'phi_min': phi_min,
               'phi_max': phi_max}
checkpoints = ((phi_min, theta_min), (phi_min, theta_max),
               (phi_max, theta_min), (phi_max, theta_max),
               (phi_min, np.pi/2), (phi_max, np.pi/2))

start = time.time()
for snap in range(98, final_snap, -1):
