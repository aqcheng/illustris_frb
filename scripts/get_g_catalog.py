...
def get_cone_shell(sim, snap, origin, outpath, checkpoints, pixels,
    ...
    for box in boxes:
        ...
        ipix = vec2pix(nside, data[:,0], data[:1], data[:,2])
        ...
        ipixs.append( ipixs[mask] )
    
    #save and write metadata
    if res: #only save if nonzero # of galaxies are found
